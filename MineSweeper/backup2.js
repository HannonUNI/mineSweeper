...
function createField(id) {
    ...
    //         let flag = false;
    //         adj = getAdjacentes(id);
    //         for (let k = 0; k < adj.length; k++) {
    //             let p = adj[k].pointToString();
    //             if (p == XYtoString(i, j)) {
    //                 flag = true;
    //                 break;
    //             }
    //         }
    //         if (getRandomInt(10) > Probability && XYtoString(i, j) != id.pointToString() && !flag) {
    //             setMine(i, j);
    //         }
    //         else
    //             field[i][j] = 0;
    ...
    /* for (let i = 0; i < height; i++)
        for (let j = 0; j < width; j++)
            if (field[i][j] != -1) {
                let adj = getAdjacentes(new Point(XYtoString(i, j)));
                for (let k = 0; k < adj.length; k++) {
                    let p = adj[k]
                    if (field[p.x][p.y] == -1)
                    field[i][j]++;
...
function flag(_id) {
    let id = _id.pointToString();
    //let cli = ['fa-solid', 'fa-flag'];
    //let flagIcon = newIcon('i', cli, id + 'f');
